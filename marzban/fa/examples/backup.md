...
اگر میخواهید از طریق ترمینال سرور و بدون نیاز به Phpmyadmin بازگردانی بکاپ دیتابیس Mysql رو انجام بدید، این روش مناسب شماست. در ابتدا نیازه که فایل بکاپ SQL خود را به سرورتون آپلود کنید، سپس در صفحه‌ی ترمینال سرورتون دستور زیر رو با جایگذاری متغیرها وارد کنید. متغیر DB_PASSWORD رو با رمزعبور دیتابیس‌تون جایگزین کنید. متغیر SQL_BACKUP_ADDRESS رو با آدرس فایل بکاپ SQL جایگزین کنید. سپس دستور را وارد کنید.
```
docker exec -i marzban-mysql-1 mysql -u root -p"DB_PASSWORD" marzban < "SQL_BACKUP_ADDRESS"
```

پس از این مرحله، نیازه که یکبار مرزبان خود را با دستور زیر ریستارت کنید.

```bash
marzban restart
```
حالا بکاپ شما با موفقیت بازگردانی شده است.
