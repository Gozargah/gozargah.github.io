...
::: warning توجه
در صورت استفاده نادرست از کوئری و یا تریگر ها هرگونه خرابی دیتابیس به عهده خود فرد می باشد ، در صورت نداشتن دانش فنی از استفاده از کوئری و علی الخصوص تریگر ها خود داری کنید.
:::
...
```
- لیست کاربرانی که `X` روز لینک سابسکریپشن خود را آپدیت نکردند 
```sql
