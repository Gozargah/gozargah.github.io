...
::: tip نکته
ترنزمیشن `H2` فقط تا ورژن `v1.8.24` هسته `Xray` پشتیبانی می‌شود و در ورژن‌های بالاتر این ترنزمیشن حذف شده‌ است و به جای آن‌ ترنزمیشن `XHTTP` جایگزین شده است.
:::
...
```
این قسمت را در `.env` پیدا کنید و با پاک کردن `#` اول آن، از حالت کامنت آن را خارج کنید و سپس مقدار `INBOUND_X` را مطابق با تگ اینباند فالبک خود قرار دهید و در نهایت برای اعمال تغییرات، مرزبان را با دستور زیر ریستارت کنید.
```bash
marzban restart
```
:::
